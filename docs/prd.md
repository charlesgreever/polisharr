...
115. As a library owner, I want Keep to rename the sidecar onto the library path when both sit on the same volume, and to clone or copy only when they do not, so that a 20 GB replace does not wait on a network round trip.
...
- Default execution: one job at a time, work on the mounted Arr paths, write sidecars on the review path. Keep places the sidecar with a same-volume rename when `stat` device ids match, then a clone, then a copy. Honor user concurrency. Optional off-peak window; a job can run now. No multi-segment encode. No local-copy-before-encode. No SSH-to-NAS or CIFS/NFS path maps.
...
- SSH-to-NAS, CIFS/NFS path maps, copy-to-local-disk before encode, and vendor control-plane copy APIs. Same-volume rename/clone of an already-written sidecar is in scope (story 115).
